...
0. 【推奨】 NVIDIA GPUを使用している方は、ドライバーをインストールする。CUDA v12.8以上であれば大丈夫です。
...
- **GPU（グラフィックボード）の推奨** CPUでの推論も可能ですが処理に非常に時間がかかるため、動画への実行は現実的ではありません。NVIDIA GPUを使用している方はCUDAの導入を強く推奨します。**AMD RadeonやIntel Arcをご使用の場合は、CPU推論のみ**となります。
