...
1. Microsoft Visual C++ 2015-2022 Redistributable (x64) のインストール。
2. Hugging Faceでアカウント作成
3. SAM3の利用許可の認証
4. アクセストークンの発行
5. プラグインをインストールして YMM4 を起動する。
6. `設定` から「SAM3 被写体切り抜き設定」を開く。
7. トークンを入力して、python環境構築
8. `ツール`から、「SAM3 被写体切り抜き」を開く。
9. 各設定を各自で設定し、出力。
...
- **動画切り抜き時のメモリ消費について** 動画を切り抜く場合は非常に多くのRAMを消費するため、**最大でも10秒程度**の素材に留めてください。RAMが32GB未満の場合、5秒程度の動画でも処理が厳しくなる可能性があります。RAMが16GBとかだと、2秒ぐらいで厳しいかも？
...
v0.2.0  
動画を読み込んだ際、RAMの使用量が多くなる問題を、正確さを犠牲にすることによって改善する設定の追加  
RAM使用量が減るだけで、処理時間は変わりません。どれだけ動画が長くても、RAM使用量が大体4GBぐらいになります。ただ、チャンクの切り替わり時に、目標が全然違うものになってしまう可能性があります。
